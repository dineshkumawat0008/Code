...
void topologicalDFS(vector<list<int>> graph, int src,vector<bool> &visited,list<int> &topologicalSortedList){
	if(!visited[src]){
		visited[src]=true;
		for(list<int>::iterator itr=graph[src].begin();itr!=graph[src].end();itr++){
			topologicalDFS(graph,*itr,visited,topologicalSortedList);
		}
...
list<int> topologicalSort(vector<list<int>> graph){
	...
	for(int src=0;src<graph.size();src++){
		topologicalDFS(graph,src,visited,topologicalSortedList);
	}
